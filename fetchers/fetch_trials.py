...
def run():
    ...
    # Exclude non-watchlist trials last updated more than 2 years ago.
    # Watchlist company trials are always kept regardless of age — older completed
    # trials are still meaningful for competitive landscape tracking.
    ...
    all_studies = {
        ...
        or is_watchlisted(s.get("sponsor", ""), watchlist_lower)
    }
    excluded = before_filter - len(all_studies)
    logging.info(f"  Excluded {excluded} non-watchlist trials updated before {cutoff_date}")
